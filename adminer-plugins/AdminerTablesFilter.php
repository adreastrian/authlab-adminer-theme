...
function tablesFilter() {
	...
	if (localStorage) {
		localStorage.setItem('adminer_tables_filter', value);
	}
	for (const table of qsa('li', qs('#tables'))) {
		...
		if (value == '') {
			...
			a.innerHTML = text.replace(reg, '<strong><i>$1</i></strong>');
		}
	}
}
...
localStorage && document.addEventListener('DOMContentLoaded', () => {
	let db = qs('#dbs').querySelector('select');
	db = db.options[db.selectedIndex].text;
	if (db == localStorage.getItem('adminer_tables_filter_db') && localStorage.getItem('adminer_tables_filter')){
		qs('#filter-field').value = localStorage.getItem('adminer_tables_filter');
		tablesFilter();
	}
	localStorage.setItem('adminer_tables_filter_db', db);
});
</script>
<p class="jsonly"><input id="filter-field" autocomplete="off" type="search" placeholder="filter tables..."><?php echo Adminer\script("qs('#filter-field').oninput = tablesFilterInput;"); ?>
<?php
	}
